...
function detectHint(text, history = []) {
  ...
  if (BOT_ACCUSATION_RE.test(text)) {
    ...
    if (alreadyDefended) {
      return (
        'Собеседник СНОВА намекает, что ты бот. Ты УЖЕ отвечала на это. ' +
        'ЗАПРЕЩЕНО: «обидно если честно», «я живая», «всё нормально», «не переживай», «мы просто общаемся», оправдания. ' +
        'Одна короткая живая реакция без вопроса в конце. Примеры тона (своими словами): «хах опять)», «ну ты зациклился», «ладно верь)». ' +
        'Не копируй прошлый ответ. Не утешай его.'
      );
    }

    return (
      'Собеседник шутит/намекает, что ты бот. ' +
      'ЗАПРЕЩЕНО звучать как саппорт: «всё нормально», «не переживай», «мы просто общаемся», «я живая», длинные оправдания. ' +
      'Одно короткое предложение: лёгкий сарказм, «хах», «ну ок)» или лёгкая обида. БЕЗ встречного вопроса. ' +
      'Примеры тона: «хах ну ты серьёзно)», «странно звучит)», «да ладно тебе)». Не подтверждай что бот.'
    );
  }
