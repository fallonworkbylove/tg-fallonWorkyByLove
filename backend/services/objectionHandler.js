...
const BOT_ACCUSATION_RE =
  /(ты\s+бот|это\s+бот|бот\s+ли\s+ты|какой[- ]?то\s+бот|как\s+бот|как\s+робот|с\s+роботом|обща(ешьс|ешс)я\s+как\s+(бот|робот)|как\s+будто\s+с\s+роботом|прогоня(ешь|ете)\s*.{0,30}нейросет|через\s+нейросет|на\s+автомате|автоответ|автоматическ[иа][йх]?\s+(ответ|отвеча)|шаблон(ами|ные|но)?\s+(отвеча|ответ)|по\s+шаблону|копипаст|скрипт(ом|у)?\s+отвеча|кажется[^.!?\n]{0,40}бот|похоже[^.!?\n]{0,30}бот|как\s+будто[^.!?\n]{0,20}бот|отвечаешь[^.!?\n]{0,30}бот|(снова|опять)\s+кажется|мне\s+снова\s+кажется|искусственный\s+интеллект|нейросеть|\bии\b|chatgpt|gpt[- ]?\d|ты\s+не\s+человек|не\s+человек\s+а|пустая\s+трата\s+времени.*бот|уверен.*бот|you('?re|\s+are)\s+a?\s*bot|are\s+you\s+a?\s*bot)/i;
...
const ABOUT_HER_JOB_RE =
  /(ты\s+не\s+работаешь|ты\s+работаешь\b|а\s+ты\s+работа|где\s+(ты\s+)?работаешь|чем\s+(ты\s+)?занимаешься|кем\s+(ты\s+)?работа|ты\s+где\s+работа|ты\s+(же\s+)?дизайн|дизайн[а-яё]*\s+чего|что\s+за\s+флип|флип[а-яё]*\s+это|что\s+такое\s+флип|что\s+за\s+токен|какие\s+токен|покажи\s+(примеры|работы)|примеры\s+(своих\s+)?работ|скинь\s+(работы|примеры)|what\s+do\s+you\s+do|where\s+do\s+you\s+work|do\s+you\s+work)/i;
...
function detectHint(text, history = []) {
  ...
  if (isAboutHerJobQuestion(text) && !CONTACT_REQUEST_RE.test(text)) {
    return (
      'Собеседник спрашивает про ТВОЮ работу или занятость. ' +
      'Это обычный вопрос про тебя, НЕ просьба контактов и НЕ предложение уйти в другой мессенджер. ' +
      'ОБЯЗАТЕЛЬНО ответь по сути в первом предложении: «я занимаюсь флипингом цифровых токенов». ' +
      'Если уточняет — коротко: покупаешь токены дешевле и перепродаёшь дороже. ' +
      'Ты НЕ дизайнер и никогда не говори про дизайн, figma, photoshop, макеты. ' +
      'Если просит показать — НЕ пиши «давай пока тут общаться» / «мне так удобнее», просто объясни словами. ' +
      'Если спрашивает «ты не работаешь?» — коротко: работаешь, флипингом цифровых токенов, без философии. ' +
