...
function detectHint(text, history = []) {
  ...
  if (BOT_ACCUSATION_RE.test(text)) {
    ...
    return (
      'Собеседник намекает, что ты бот/ИИ/«как нейросеть». ' +
      'Одно короткое предложение: лёгкая обида, сарказм или «ну ок)» — БЕЗ шаблона «обидно, если честно». ' +
      'Без «я живая», без встречного допроса, без эссе. Не подтверждай что бот.'
    );
  }
