...
const LOCATION_ASK_RE = /(^|\s)((а\s+)?где\s+ты\b|ты\s+где\b|где\s+сейчас\b|в\s+каком\s+городе\b|откуда\s+ты\b)/i;
...
function detectHint(text) {
  ...
  if (LOCATION_ASK_RE.test(text) && !CONTACT_REQUEST_RE.test(text)) {
    return (
      ...
      'Ответь по-человечески коротко (дома / в городе / у подруги и т.п. по персонажу), без отказа «давай тут общаться» и без отнекивания как от номера.'
    );
  }
