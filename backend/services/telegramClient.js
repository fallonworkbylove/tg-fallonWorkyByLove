...
function getWorkZoneHour() {
  try {
    ...
    // Некорректный часовой пояс — запасной вариант: М��К = UTC+3.
  }
...
// NFT-голосовое разрешено только со второго дня общения.
// Первый день — первые 24 часа после самого раннего сообщения — всегда исключён.
const NFT_VOICE_AFTER_HOURS = 24;
...
async function getNftCampaignState(accountId, peerId, historyLength) {
  ...
  // Со второго дня: пора просить помощи с токеном голосовым. В первый день
  ...
  if (ageHours >= NFT_VOICE_AFTER_HOURS) {
    ...
    return {
      hint:
        'Вы уже общаетесь не первый день. Ты уже рассказывала, что зарабатываешь на NFT. ' +
  ...
  // 1–2 день: мягко напоминаем про тему, но не чаще ��аза в сутки.
