...
// Модель для чата (ответы). Меняется через .env: OPENAI_MODEL=gpt-5.6-sol.
// 2026-08-22: тест на сутки — переведено с gpt-4o-mini на gpt-4o (ответы живее
// и лучше держат характер, но запрос дороже mini). Чтобы вернуть mini, достаточно
// задать OPENAI_MODEL=gpt-4o-mini в .env (перебивает значение по умолчанию).
  ...
  if (options.mediaEnabled) {
    messages.push({
      ...
      content:
        ...
        'Если просит кружок/видеосообщение или ты сама хочешь показать, чем занимаешься — токен <<CIRCLE>>. ' +
