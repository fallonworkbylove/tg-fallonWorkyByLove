...
async function generateReply(systemPrompt, history, userMessage, options = {}) {
  ...
  // собеседника и мягко подстраивает ��тиль ответа — без явного
  ...
  if (options.mediaEnabled) {
    messages.push({
      ...
      content:
        ...
        'НЕ должен быть отказом или от��ов��ркой. Никаких «п��ка ��ано», «не могу», «рано», «попозже», ' +
        '«не кидаю» вместе с токеном. Отправляешь — значит соглашаешься: пиши дружелюбно и коротко ' +
        '(«ща», «щас», «��кей держи», «ну лови») или вообще без текста, только токен. ' +
        ...
        'новый токен. Новый токен вст��вляй только если человек СНОВА явно просит прислать ещё. ' +
